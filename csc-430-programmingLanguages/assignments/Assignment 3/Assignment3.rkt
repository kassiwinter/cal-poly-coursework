  ...
  (match s
    ...
    [(? symbol? sym)
     (cond
       ...
       [else (IdC sym)])]
    [(list (? symbol? fun) (cons arg '())) (AppC fun (parse arg))]
    [(list (? symbol? sym) l r)
     (cond
...
;[other (error 'parse "PAIG Error. Unsupported S-Expression" other)]
  ...
  (match s
   [(list 'fun (list (? symbol? id) (list (? symbol? arg))) body)
    (cond
     ...
     [else (FundefC id arg (parse body))])]
...
(check-equal?
  (parse-fundef '{fun {f (x)} {+ x 1}})
  (FundefC 'f 'x (BinopC '+ (IdC 'x) (NumC 1))))
...
(check-exn (regexp (regexp-quote "PAIG Error. Invalid function definition."))(lambda ()(parse '+)))
(check-exn exn:fail?
  (lambda ()
    (DivC 5 0)))
