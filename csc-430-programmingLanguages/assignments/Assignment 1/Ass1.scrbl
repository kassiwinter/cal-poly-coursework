...
For these reasons and more, I would suggest using **C++** as the main
programminglanguage when developing. This language is primarily known
for its fine-graincontrol over memory and resources, making it perfect
for not only safety-criticalsystems (such as self-driving cars), but for
ensuring minimal runtime and delay. Further, because of its strong hardware
interaction ability, this language is capable of extracting and handling
extensive amounts of data that are intaken from images, videos, and other
object direction media. C++ is also praised for its support of AI libraries
such as Tensorflow and OpenCV, which can enable instantaneous operations
and perception decision-making for machine learning. It is also important
to note that C++ is a highly demanded and popular language that most
developers on the team would either know or could easily pick up. However,
if for some reason the board deems the C++ language unacceptable for industry
standards, I would urge the use of **Rust**. This is due to the language
being a balanceof performance, safety, and reliability (similar to C++’s
ability), while still being able to hold unexpected errors and technological
advancements that may occur during development, ensuring quality. Nonetheless,
it is important to keep in mindthat developing using Rust will cost more in
resources and production rather than C++.
