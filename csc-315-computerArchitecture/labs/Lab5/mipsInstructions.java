...
/* #1 Checks to see if a name is valid; returns T if valid, F if not */
    ...
    /* #3 Do instructions */
    public static int processInstruction(mipsEmulator emulator, ArrayList<String> instructions, HashMap<String, Integer> labels, int programCounter) {
        String line = instructions.get(programCounter);
		...
		switch (instruction[0]){
			...
			case("beq"):
                emulator.branchPredict.numTotalPredictions++;
				if(emulator.getValue(instruction[1]) == emulator.getValue(instruction[2])) {
					if (emulator.branchPredict.getPrediction() == true) {
					    emulator.branchPredict.numCorrectPredictions++;
                    }
                    emulator.branchPredict.updatePrediction(true);
                    programCounter = labels.get(instruction[3].trim());
				} else {
                    if (emulator.branchPredict.getPrediction() == false) {
					    emulator.branchPredict.numCorrectPredictions++;
                    }
                    emulator.branchPredict.updatePrediction(false);
					programCounter++;
				}
				break;
