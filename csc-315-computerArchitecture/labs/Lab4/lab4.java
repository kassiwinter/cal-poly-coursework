...
public class lab4 {
	...
	try {
		...
		if (args.length == 2) {
			...
			scriptMode(script, emulator, instructions, labels, cpu);
		}	 
        
        	// otherwise go to interactive mode as default
    		else {
			interactiveMode(emulator, instructions, labels, cpu);
		}	
		scanner1.close();
		scanner2.close();
	}	
    ...
    /* --- SCRIPT MODE --- */
	public static void scriptMode(File file, mipsEmulator emulator, ArrayList<String> instructions, HashMap<String, Integer> labels, CPU cpu) {
		try{
			...
			while(scanner.hasNextLine()){
				...
				programCounter = getCommand(line.trim(), emulator, programCounter, instructions, labels, cpu);
				if (programCounter == -1) {
					break;
				}
			}
     ...
     /* --- INTERACTIVE MODE --- */
    public static void interactiveMode(mipsEmulator emulator, ArrayList<String> instructions,  HashMap<String, Integer> labels, CPU cpu){
	try{
        	...
        	while(programCounter != -1) {
            		...
            		programCounter = getCommand(input, emulator, programCounter, instructions, labels, cpu);
        	}
		scanner.close();
	}
	catch(Exception e) {
	...
	public static int getCommand(String input, mipsEmulator emulator, int programCounter, ArrayList<String> lineList, HashMap<String, Integer> labels, CPU cpu) {
		switch(input.charAt(0)) {
			...
			case ('p'):
				...
				programCounter + "\t" + cpu.regs.get("if/id reg") + "\t" + 
				cpu.regs.get("id/exe reg") + "\t" +
				cpu.regs.get("exe/mem reg") + "\t" + 
				cpu.regs.get("mem/wb reg"));
				break;
                ...
                if (input.length() > 1) {
                    ...
                    while (i < step) {
                        if (programCounter < lineList.size()) {
                            programCounter = mipsInstructions.processInstruction(emulator, lineList, labels, programCounter, cpu);
							System.out.println("pc\tif/id\tid/exe\texe/mem\tmem/wb\n" + 
							programCounter + "\t" + 
							cpu.regs.get("if/id reg") + "\t" + 
							cpu.regs.get("id/exe reg") + "\t" +
							cpu.regs.get("exe/mem reg") + "\t" + 
							cpu.regs.get("mem/wb reg")+"\n");
                        }
